...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await ensurePrintifyProductsForListings(listingIds, { waitForMockupsMs: 8000 });
    const mockups = await refreshPrintifyListingMockups();
    return NextResponse.json({
      ...result,
      mockupsUpdated: Math.max(result.mockupsUpdated, mockups.updated),
    });
  } catch (err) {
